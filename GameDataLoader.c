...
int load_level_data() {
	...
	if (SDL_GetCurrentDisplayMode(0, &mode) != 0) {
		printf("Could not get Display Mode");
		return -1;
	}
	...
	if (!config_lookup_int(&level_data, "tiles_x", &tiles_x)) {
		tiles_x = 16;
	}
	if (!config_lookup_int(&level_data, "tiles_y", &tiles_y)) {
		tiles_y = 9;
	}
	texture_width = width / tiles_x;
	texture_height = height / tiles_y;
	...
	snake = insert_new_texture(snake, x * texture_width, y * texture_height, get_clipping_window(type), get_game_texture(type[0]), texture_width * get_x_sign(type[1]), texture_height * get_y_sign(type[1]), get_direction(type[1]));
	last = snake;
	...
	for (itr = 0; itr < len; itr++) {
		...
		snake = insert_new_texture(snake, x * texture_width, y * texture_height, get_clipping_window(type), get_game_texture(type[0]), texture_width * get_x_sign(direction[0]), texture_height * get_y_sign(direction[0]), get_direction(direction[0]));
	}
	...
	if (len > 0) {
		...
		for (itr = 0; itr < len; itr++) {
			child = config_setting_get_elem(dat, itr);
			obstacles[itr].x = texture_width * config_setting_get_int_elem(child, 0);
			obstacles[itr].y = texture_height * config_setting_get_int_elem(child, 1);
		}
	}
	return 1;
}
